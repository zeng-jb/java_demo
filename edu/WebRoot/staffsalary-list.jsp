...
<html>
  ...
  <head>
	...
	function goPage(cp, params) {
		var s;
		if (params == "") {
			s = "StaffWageServlet.action?&op=findStaffWageByPage&cp=" + cp;
		} else {
			s = "StaffWageServlet.action?op=findStaffWageByPage&cp=" + cp + "&"	+ params;
		}
		$.post(
				s,//请求地址和参数
				function(data) {
						...
						for (var i = 0; i < list.length; i++) {
							tabData += '<tr>';
								...
								tabData += '<td>' + list[i].arrangewage + '</td>';
								tabData += '<td>' + list[i].senioritywage + '</td>';
								tabData += '<td>' + list[i].fine + '</td>';
								tabData += '<td>' + list[i].deductwage + '</td>';
								tabData += '<td>' + list[i].totalwage + '</td>';
								tabData += '<td>' + jsonDateFormat(list[i].month)+ '</td>';
								//注意转义字符  操作
								tabData += '<td class="td-manage">'
										+ '<a title="详情" onclick="x_admin_show(\'详情\',\'StaffWageServlet.action?op=findStaffWageById&id='
										+ list[i].staffid
