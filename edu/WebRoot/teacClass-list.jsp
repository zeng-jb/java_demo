	...
	function mysearch(pagenumber){
	 ...
	 //分页函数
		function goPage(cp,params) {
			    var s ;
			    if(params == ""){
			    	s="TeacClassInfoServlet.action?&op=findClassInfoByPage&cp="+cp+"&teacherid="+${sessionScope.login.teacherid};
			    }else{
			    	 s="TeacClassInfoServlet.action?op=findClassInfoByPage&cp="+cp+"&teacherid="+${sessionScope.login.teacherid}+"&"+params;
			    }
				...
				for(var i = 0; i<list.length;i++){
					...
					tabData += '<td>'+jsonDateFormat(list[i].createtime)+'</td>';
						if(list[i].status=='正常状态'){
							...
							//注意转义字符
							tabData += '<td class="td-manage"><a title="登录成绩" onclick="x_admin_show(\'学生成绩\',\'PageTransServlet.action?op=pageTrans&classid='+list[i].classid+'\',900,400)" href="javascript:;"> <i class="layui-icon">&#xe608;</i></a><a title="查看成绩" onclick="x_admin_show(\'学生成绩表\',\'PageTransServlet.action?op=pageTrans2&classid='+list[i].classid+'\',900,400)" href="javascript:;"> <i class="layui-icon">&#xe63c;</i></a></td>';
						}else{
